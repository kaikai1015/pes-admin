...
@Component("cuxMaterialTransactionsJob")
public class CuxMaterialTransactionsJob {
    @Autowired
    ...
    public void report() throws Exception {
        ...
        log.setJobName("CuxMaterialTransactionsJob");
        ...
        byte[] data = DataModelCommon.getData("/SYNC_DATA/CuxWorkMetrialTrnData_externalPes.xdm",params);
        ...
        if (true) {//MATERIAL_TRANSACTIONS
            ...
            List<CuxMaterialTransactionsEO> list = new ArrayList<CuxMaterialTransactionsEO>();
            for (int i = 0; i < dataSize; i++) {
                Node node = nodeList.item(i);
                NodeList dataList = node.getChildNodes();
                //遍历每条数据的字段信息
                CuxMaterialTransactionsEO o = new CuxMaterialTransactionsEO();
            ...
            cuxMaterialTransactionsMapper.deleteCuxMaterialTransactionsEO(list);
            if (list.size() > 0) {
                ...
                for (int index = 0; index < list.size();) {
                    if (batchLastIndex > (list.size() - 1)) {
                        ...
                        cuxMaterialTransactionsMapper.batchCuxMaterialTransactionsEO(list.subList(index, (batchLastIndex + 1)));
                        break;// 数据插入完毕,退出循环
                    }else {
                        cuxMaterialTransactionsMapper.batchCuxMaterialTransactionsEO(list.subList(index, (batchLastIndex + 1)));
